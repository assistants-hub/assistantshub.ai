          ...
          {model?.features?.retrieval ? (
            <div>
              <p className={'text-sm text-gray-400'}>
                Upload documents that will be used for retrieval function by
                your assistant
              </p>
              <DocumentsManager />
            </div>
          ) : (
            <div className='bg-grey flex h-[calc(100vh-120px)] flex-col items-center justify-center'>
              <HiOutlineFolder className='text-9xl text-gray-400 dark:text-gray-700' />
              <p className='text-gray-400 dark:text-gray-400'>
                Support for documents is not available for{' '}
                <b>{model ? model.id : 'this model'}</b> yet.
              </p>
            </div>
          )}
        </div>
